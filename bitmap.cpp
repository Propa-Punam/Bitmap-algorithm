...
//        cout<<endl;
   ...
   myFile<<"ID"<<","<<"NAME"<<","<<"SCORE"<<"\n";
    ...
    {
        //cout << (*i).first<<"\t"<<id_name[(*i).first]<<"\t\t\t"<< (*i).second <<endl;
        //outFile << (*i).first-1<<"\t"<< (*i).second <<endl;

        myFile<<(*i).first<<","<<id_name[(*i).first]<<","<<(*i).second<<"\n";
...
void make_dataset(vector<vector<double>>&dataset,map<int,string>& id_name,int dimension)
{
   ...
   // ifstream inFile;
    ...
    ofstream myFile("output.csv");
     myFile<<"Name"<<","<<"GP"<<","<<"PPGPointsPoints per game"<<","<<"RPGReboundsRebounds per game"<<","<<"APGAssistsAssists per game"<<"\n";
...
myFile<<name<<","<<gp<<","<<pts<<","<<reb<<","<<ast<<"\n";
             ...
             id_name.insert(pair<int,string>(id, name));
              vector<double>m{id,gp,pts,reb,ast};
            dataset.push_back(m);
